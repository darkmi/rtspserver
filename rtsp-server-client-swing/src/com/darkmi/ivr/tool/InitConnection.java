...
import com.darkmi.ivr.biz.IvrHander;
import com.darkmi.ivr.protocol.MessageCodecFactory;
...
public class InitConnection {
	 ...
	 * @return
	 */
	public IoSession[] initConn(int count,int a) {
		IoSession[] list = new IoSession[count];
		for (int i = 0; i < count; i++) {
			IoSession sess = this.udpconn();
			if (sess != null)
				list[i] = sess;
		}
		return list;
	...
	private IoSession udpconn() {
		try {
			IoSession session = new MinaUtil().ConnUdpByMina(TestEntity.IVRIP,
					TestEntity.IVRPort, new IvrHander(),
	...
	private IoSession conn() {
		try {
			IoSession session = new MinaUtil().ConnTcpByMina(TestEntity.IVRIP,
					TestEntity.IVRPort, new IvrHander(),
					new MessageCodecFactory());
			return session;
		} catch (Exception e) {
